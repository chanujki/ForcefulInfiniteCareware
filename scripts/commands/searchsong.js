...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (!link)
    return api.sendMessage('[⚠️]➡️ Please provide an image or video link.', event.threadID, event.messageID);

  try {
    const cleanedLink = link.trim().replace(/\s/g, '');
    if (!/^https?:\/\//.test(cleanedLink)) {
      return api.sendMessage('[⚠️]➡️ Invalid URL: must start with http:// or https://', event.threadID, event.messageID);
    }
    ...
    if (!data || data.length === 0) {
      return api.sendMessage('[⚠️]➡️ No results found for this media.', event.threadID, event.messageID);
    }
    ...
    console.error(error);
    return api.sendMessage('[⚠️]➡️ Error fetching data.', event.threadID, event.messageID);
  }
};
...
module.exports.handleReply = async ({ api, event, handleReply }) => {
  ...
  if (!step) {
    ...
    if (isNaN(choice) || choice < 1 || choice > results.length) {
      return api.sendMessage("[⚠️]➡️ Invalid choice. Please reply with a valid number.", event.threadID, event.messageID);
    }
    ...
    if (reply !== '1' && reply !== '2') {
      return api.sendMessage("[⚠️]➡️ Please reply with 1 for audio or 2 for video.", event.threadID, event.messageID);
    }
    ...
    try {
      ...
      if (!data?.status || !data?.data) {
        return api.sendMessage("[⚠️]➡️ Failed to fetch download URL.", event.threadID, event.messageID);
      }
      ...
      console.error(err);
      return api.sendMessage("[⚠️]➡️ Error while downloading media.", event.threadID, event.messageID);
    }
  }
};
