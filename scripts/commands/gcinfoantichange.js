...
const activeGroupsFilePath = path.join(__dirname, "..", "events", "Rakib", "groupSettings.json");
...
module.exports = {
  ...
  start: async function ({ Rakib, events, args, Threads }) {
    const threadID = events.threadID;
    const senderID = events.senderID;
    const botAdmins = global.config.ADMINBOT;
 
    // Check if the user is an admin or bot admin
    const threadInfo = await Rakib.getThreadInfo(threadID);
    const groupAdmins = threadInfo.adminIDs.map(admin => admin.id);
 
    if (!groupAdmins.includes(senderID) && !botAdmins.includes(senderID)) {
      return Rakib.sendMessage("⚠️ Only group admins or bot admins can use this command.", threadID);
    }
 
    const initialThreadInfo = await Rakib.getThreadInfo(threadID);
    ...
    if (args[0] === "on") {
      ...
      if (!activeGroups[threadID]) {
        ...
        saveActiveGroups();
        Rakib.sendMessage("✅ Anti-change feature has been activated for this group.", threadID);
      } else {
        Rakib.sendMessage("⚠️ Anti-change feature is already active for this group.", threadID);
      }
    } else if (args[0] === "off") {
      if (activeGroups[threadID]) {
        ...
        saveActiveGroups();
        Rakib.sendMessage("🚫 Anti-change feature has been deactivated for this group.", threadID);
      } else {
        Rakib.sendMessage("⚠️ Anti-change feature is not active for this group.", threadID);
      }
    } else {
      Rakib.sendMessage("⚠️ Invalid option. Please use 'antichange on' or 'antichange off'.", threadID);
    }
  }
};
